...
const HistoryRecipeCard: React.FC<HistoryRecipeCardProps> = ({ historyItem }) => {
  ...
  if (!source_url) {
    return (
      <div className={`${styles.card} ${styles.noLink}`}>
        <img src={imageUrl} alt={recipe_data.title || 'Receta'} className={styles.cardImage} loading="lazy" />
        {is_adapted && <span className={styles.aiBadge}>Adaptado por IA</span>}
        <div className={styles.cardContent}>
          <h3 className={styles.cardTitle}>{recipe_data.title || 'Receta sin título'}</h3>
  ...
  return (
    <Link to={`/recipe/details?url=${encodeURIComponent(source_url)}`} className={styles.cardLink}>
